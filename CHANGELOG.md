...
- **`afterRender` lifecycle hook.** Fires after every render (initial and
  subsequent). Use it for imperative DOM work that needs to run on every render
  — e.g. calling `dialog.showModal()` on a freshly-rendered `<dialog>`. The
  kit's `<tc-modal>` uses this to fix a bug where setting `open = true` after
  the initial render didn't actually open the dialog.
