...
[tan-compose](https://jsr.io/@ra9/tan-compose) ecosystem. Ships 98 common icons
...
security, social, people, plus 10 originals (`ai`, `confetti`,
`forecast`, `pulse`, `receipt-scan`, `recurring`, `spark`,
`subscription`, `token`, `verified`) tuned for fintech moments that
Lucide doesn't quite hit:

```text
ai alert-circle alert-triangle arrow-down arrow-left arrow-right arrow-up
...
confetti copy credit-card dollar-sign download edit external-link eye
eye-off file-text filter folder forecast gift globe grid heart home image
info key landmark link list loader lock log-in log-out mail map-pin menu
message-square minus moon more-horizontal more-vertical package paperclip
percent phone pie-chart piggy-bank plus printer pulse receipt receipt-scan
recurring refresh save search send settings shield shopping-bag
shopping-cart spark star subscription sun tag token trash trending-down
trending-up truck unlock upload user user-check user-plus users verified
wallet x x-circle
```
...
The full bundle (component + 98 icons) is about 20 KB minified (~8 KB gzipped).
